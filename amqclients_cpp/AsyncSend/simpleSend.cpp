...
class simpleSend : public proton::messaging_handler {
  private:
    std::string url;
    std::string user;
    std::string password;
    ...
    int total;
  public:
    simpleSend(const std::string &s, const std::string &u, const std::string &p, int c) :
    ...
    std::cout << "credits before send: "<< s.credit() << std::endl;
      ...
      std::cout << "credits after send: "<< s.credit() << std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    try {
        opts.parse();
        simpleSend send(address, user, password, message_count);
